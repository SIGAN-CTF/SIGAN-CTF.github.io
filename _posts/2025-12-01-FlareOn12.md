...
Welcome to my [Flare-On](https://flare-on.com/) writeups! This is my first serious attempt to fully solve this CTF. I'm very happy that it was successful. Now I need to work on solving tasks faster, since I might not even have time to solve the last one this year. And if you look at the time it took to solve the tasks, you'll notice a huge difference between the time it took to solve the last one 🙂:

![](/assets/img/flareon.png)

Still, it was a valuable experience - even though this was the first year Russians were excluded from the list of countries that solved all CTF tasks 😭
...
New C2 server binary uses **NEW ENCRYPTION SCHEME at `0x140050D20` 🙂🙃 AND NEW KEY EXCHANGE 🙂🙃 at `0x140081300`** 🔥🔥🔥
...
Since the calculation of the hash depend only on index of the digit in the password, we can create a table with the hash values ​​at each position (that can help us to skip reimplementing for hashing function).
...
After calling f-functions Dll CRC is processed inside a loop where it is “spread” into a large matrix using row-wise XOR operations:
...
- Load the 32×64-bit S-box values (sbox[0..31]) into memory. These values define a permutation of 0–255 for byte substitution.
...
Function performs a 32-byte bytewise shuffle (permutation) using a small 32-byte table derived from 5×64-bit constants:

![](/assets/img/shuffle.png)

Here's what the function does:
- Like previous permutations here performes initial XOR on the first 4 bytes with `TARGET_TABLE`;
- Build the shuffle table `v2_bytes` from the 5×64-bit constants (table[0..4]), converting each QWORD into 8 bytes. This defines a fixed byte permutation of length 32.
...
File `assigned_indices.txt` contains idexes at line number as dll_id 🙂. We can generate `license.bin` from this and validate that everything is correct:
```python
...
class DllManager:
    ...
    def _process_dll(self, dll):
        """Возвращает tuple (dll_name, check_entry, ffunc_entries) или None при ошибке"""
        try:
